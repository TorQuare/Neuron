...
for i in file:
    ...
    if int(percent_arr[len(percent_arr)-1]) == percent:
        if int(S_arr_len[len(S_arr_len)-1]) == S_length and len(after_ga_values_15) < 10:
            bef_ga_values.append(float(bef_val[len(bef_val)-1])*100)
            after_ga_values_15.append(float(aft_val[len(bef_val)-1])*100)
        if int(S_arr_len[len(S_arr_len)-1]) == S_length + 10 and len(after_ga_values_25) < 10:
            after_ga_values_25.append(float(aft_val[len(bef_val)-1])*100)
        if int(S_arr_len[len(S_arr_len)-1]) == S_length + 20 and len(after_ga_values_35) < 10:
            after_ga_values_35.append(float(aft_val[len(bef_val)-1])*100)
        if int(S_arr_len[len(S_arr_len)-1]) == S_length + 30 and len(after_ga_values_45) < 10:
            after_ga_values_45.append(float(aft_val[len(bef_val)-1])*100)
